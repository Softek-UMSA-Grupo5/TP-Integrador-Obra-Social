...
const AddNewOfficeForm = () => {
    ...
    const diaSemanaValue = value as DiaSemana; 
    setOfficeData((prevData) => ({
        ...prevData,
        horariosDeAtencion: prevData.horariosDeAtencion.map((horario, idx) =>
            idx === index ? { ...horario, diaSemana: diaSemanaValue } : horario
        ),
    }));
};
    ...
    const handleCreateNewMedico = async () => {
        try {
            // Simulación de creación de nuevo médico
            const newMedicoId = await createMedicoAndReturnId(officeData.medico);
            setOfficeData((prevData) => ({
                ...prevData,
                medico: newMedicoId,
            }));
    ...
    const handleSelectExistingMedicoChange = (event: SelectChangeEvent<string>) => {
        ...
        const selectedMedico = existingMedicos.find((medico) => medico.id === parseInt(event.target.value));
        if (selectedMedico) {
            setOfficeData((prevData) => ({
                ...prevData,
                medico: {
