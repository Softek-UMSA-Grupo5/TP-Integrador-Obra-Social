...
const ConsultorioForm: React.FC = () => {
    ...
    const handleHorarioChange = (index: number, event: ChangeEvent<HTMLInputElement>) => {
        ...
        // No aplicamos formatTimeInput directamente aquí
        const updatedHorarioAtencion = officeData.horarioAtencion.map((horario, i) =>
            i === index ? { ...horario, [name]: value } : horario
        );

        // Actualizamos officeData asegurando que solo se aplique addSecondsToTime si es necesario
        setOfficeData((prevData) => ({
            ...prevData,
    ...
    const handleBlur = (index: number, event: React.FocusEvent<HTMLInputElement>) => {
        const { name, value } = event.target;
        const formattedValue = formatTimeInput(value); // Aplicamos formatTimeInput cuando el usuario termina la edición
