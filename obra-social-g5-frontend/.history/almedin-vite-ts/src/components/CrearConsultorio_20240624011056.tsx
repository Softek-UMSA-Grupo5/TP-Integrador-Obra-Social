...
const AddNewOfficeForm = () => {
    ...
    const handleSelectExistingMedicoChange = (event: SelectChangeEvent<string>) => {
        ...
        if (selectedMedico) {
            setOfficeData((prevData) => ({
                ...prevData,
                medico: selectedMedico.id, // Aquí se guarda el ID del médico seleccionado
            }));
        }
    };
    ...
    const handleNoMedicoSelection = () => {
        setOfficeData((prevData) => ({
            ...prevData,
            medico: initialMedico.id, // Aquí se restaura el ID del médico a su valor inicial
        }));
    };
    return (
