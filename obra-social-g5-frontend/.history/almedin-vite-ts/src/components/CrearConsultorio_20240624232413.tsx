...
const ConsultorioForm: React.FC = () => {
    ...
    const handleSelectExistingMedicoChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        setSelectedMedicoId(event.target.value as unknown as number);
    };
    ...
    const handleHorarioChange = (index: number, event: ChangeEvent<HTMLInputElement>) => {
        ...
        // Formatear la hora ingresada (asegura que minutos y segundos sean 00)
        const formattedTime = formatTimeInput(value);

        // Actualizar el horario de atención con la hora formateada
    ...
    const formatTimeInput = (value: string): string => {
        // Limpia la entrada para permitir solo números y ':'
        const cleanedValue = value.replace(/[^\d:]/g, '');

        // Divide la entrada para separar horas, minutos y segundos
        const parts = cleanedValue.split(':');

        // Obtener las partes de la hora
        let hours = parts[0] || '00';

        // Establecer los minutos y segundos en 00 si no están especificados
        let minutes = parts.length > 1 ? parts[1] : '00';
        let seconds = parts.length > 2 ? parts[2] : '00';

        // Formatear las horas asegurándose de tener dos dígitos y que no sea mayor a 23
        hours = String(Math.min(parseInt(hours, 10), 23)).padStart(2, '0');

        // Formatear los minutos asegurándose de tener dos dígitos y que no sea mayor a 59
        minutes = String(Math.min(parseInt(minutes, 10), 59)).padStart(2, '0');

        // Formatear los segundos asegurándose de tener dos dígitos y que no sea mayor a 59
        seconds = String(Math.min(parseInt(seconds, 10), 59)).padStart(2, '0');

        // Retornar la hora formateada en el formato HH:mm:ss
