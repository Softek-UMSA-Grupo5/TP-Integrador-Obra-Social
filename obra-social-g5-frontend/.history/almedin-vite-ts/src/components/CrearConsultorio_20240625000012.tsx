...
const ConsultorioForm: React.FC = () => {
    ...
    const handleHorarioChange = (index: number, event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        const { name, value } = event.target;
    ...
    const formatTimeInput = (value: string): string => {
        // Limpiar el valor para mantener solo números y dos puntos
        ...
        if (cleanedValue.length > 0) {
            hours = cleanedValue.slice(0, 2).padEnd(2, '0');
            minutes = cleanedValue.slice(2, 4).padEnd(2, '0');
        }

        // Asegurarse de que las horas y minutos están dentro de los límites permitidos
        hours = String(Math.min(parseInt(hours, 10), 23)).padStart(2, '0');
        minutes = String(Math.min(parseInt(minutes, 10), 59)).padStart(2, '0');

        // Formatear con segundos fijos a '00'
                                    ...
                                    <FormControl>
                                        ...
                                        <Input
                                            ...
                                            onChange={(e: ChangeEvent<HTMLInputElement>) =>
                                                handleHorarioChange(0, e)
                                            }
                                            onBlur={(e) => handleHorarioBlur(0, e)}
                                        />
