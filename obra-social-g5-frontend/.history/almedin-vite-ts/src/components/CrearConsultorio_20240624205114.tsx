...
const ConsultorioForm: React.FC = () => {
    ...
    const { name, value } = event.target;
    const cleanedValue = formatTimeInput(value); // Aplica limpieza y formato HH:mm
    const updatedHorarioAtencion = officeData.horarioAtencion.map((horario, i) =>
        i === index ? { ...horario, [name]: cleanedValue } : horario
    );

    // Actualiza officeData asegurando que solo se aplique addSecondsToTime si es necesario
    setOfficeData((prevData) => ({
        ...prevData,
    ...
    const formatTimeInput = (value: string): string => {
        ...
        // Formatea las horas y los minutos
        const hours = parts[0] ? String(Math.min(parseInt(parts[0], 10), 23)).padStart(2, '0') : '';
        const minutes = parts[1]
            ? String(Math.min(parseInt(parts[1], 10), 59)).padStart(2, '0')
            : '';
        // Retorna el formato HH:mm
