...
import { getAllMedicos, addMedico } from '../../axios/MedicoApi';
...
const UsuarioForm: React.FC = () => {
    ...
    const [selectedMedicoId, setSelectedMedicoId] = useState<number>(-1); // Valor inicial por defecto
        ...
        try {
            if (isNewMedico) {
                ...
                if (medicoExistente) {
                    ...
                    await addMedico([medicoExistente]);
                }
            }
    ...
    /* const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            if (isNewMedico) {
                ...
                console.log('Registrando usuario con datos:', usuarioData, 'y rol:', selectedRol);
                await registrarUsuario(usuarioData, selectedRol);
            } else if (isExistingMedico) {
                const medicoExistente = medicos.find((medico) => medico.id === selectedMedicoId);
                if (medicoExistente) {
                    ...
                    // Asegura que el rol seleccionado esté actualizado correctamente
                    console.log(
                        'Registrando usuario con datos:',
                        usuarioData,
                        'y rol:',
                        selectedRol
                    );
            ...
            setError('Error al registrar el usuario o el médico');
        }
    }; */
