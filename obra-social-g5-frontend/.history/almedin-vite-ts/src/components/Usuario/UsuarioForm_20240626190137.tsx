...
import { ConsultorioResponseDto, ConsultorioCreateRequest } from '../../types/Consultorio';
import { SelectChangeEvent } from '@mui/material/Select';
import { MedicoRequestDto } from '../../types/Medico';
import { addMedico, getAllMedicos } from '../../axios/MedicoApi'; // Ajusta la importación según tu estructura
...
const UsuarioForm: React.FC = () => {
    ...
    const fetchConsultorios = async () => {
        try {
            const response = await getAllConsultorios();
            // Verificar si la respuesta tiene una propiedad 'data'
            if ('data' in response) {
                const consultoriosData = response.data; // Acceder a 'data' de manera segura
                if (Array.isArray(consultoriosData)) {
                    setConsultorios(consultoriosData);
                } else {
                    console.error('El formato de datos de consultorios no es válido');
                }
            } else {
                console.error('La respuesta no contiene una propiedad "data" esperada');
            }
        } catch (error) {
