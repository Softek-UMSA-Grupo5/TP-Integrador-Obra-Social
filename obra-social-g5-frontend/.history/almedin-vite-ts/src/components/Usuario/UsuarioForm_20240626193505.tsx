...
const UsuarioForm: React.FC<UsuarioFormProps> = ({ existingMedicos, consultorios }) => {
    ...
    const [medicos, setMedicos] = useState<MedicoResponseDto[]>([]); // Estado para almacenar médicos
    const [consultoriosState, setConsultorios] = useState<ConsultorioResponseDto[]>([]); // Estado para almacenar consultorios
    ...
    const fetchConsultorios = async () => {
        try {
            ...
            const consultoriosResponseDto: ConsultorioResponseDto[] = consultoriosData.map(
        ...
        try {
            ...
            if (
                ...
                const medicoData: MedicoRequestDto[] = [
                    {
                        nombre:
                            existingMedicos.find((medico) => medico.id === selectedMedicoId)
                                ?.nombre ?? '',
                        apellido:
                            existingMedicos.find((medico) => medico.id === selectedMedicoId)
                                ?.apellido ?? '',
