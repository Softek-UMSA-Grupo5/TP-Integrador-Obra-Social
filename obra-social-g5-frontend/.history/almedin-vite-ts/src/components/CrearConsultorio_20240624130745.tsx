...
import { createConsultorio, updateConsultorio } from '../api/ConsultorioApi';
...
const ConsultorioForm: React.FC = () => {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const consultorioResponse: any = await createConsultorio(officeData); // Ajusta el tipo según la respuesta real
            const consultorioId = consultorioResponse.data.id;

            // Paso 2: Solo si se está creando un nuevo médico
            if (createNewMedico) {
                // Crear el médico con el consultorioId asociado
                const medicoToCreate: MedicoCreateRequest = {
                    // Ajusta con los datos correctos del médico según tu estructura
                    nombre: '',
                    apellido: '',
                    telefono: '',
                    email: '',
                ...
                const medicoResponse = await createMedico(medicoToCreate); // Ajusta con la función real para crear médicos
                const medicoId = medicoResponse.data.id; // Asegúrate de ajustar la respuesta de acuerdo a tu API

                // Paso 3: Actualizar el consultorio con el medicoId (si es necesario en tu lógica)
                await updateConsultorio({ id: consultorioId, medicoId }); // Ajusta según tu API
            ...
            // Llamar al callback de éxito del formulario si existe
            // onSubmitSuccess(); // Asegúrate de definir esta función en tu componente
                            ...
                            <Grid container spacing={2} key={index} my={2} ml={3}>
                                <Grid item xs={4}>
                                    <FormControl fullWidth variant="outlined">
                                        <InputLabel shrink sx={{ my: 0.5 }}>Día de la Semana</InputLabel>
                                        <Select
                                            name="diaSemana"
