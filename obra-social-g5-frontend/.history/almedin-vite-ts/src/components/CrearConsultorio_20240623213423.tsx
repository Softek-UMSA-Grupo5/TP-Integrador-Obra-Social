...
interface Medico {
    id: number; // Agregamos el campo 'id' para identificar al médico seleccionado
    nombre: string;
    apellido: string;
    telefono: string;
    email: string;
...
const [existingMedicos, setExistingMedicos] = useState<Medico[]>([
  {
    id: 2,
    nombre: 'Luciano',
    apellido: 'Malleret',
...
const AddNewOfficeForm = () => {
    ...
    const initialMedico: Medico = {
        id: 0, // Campo 'id' inicializado como vacío
        nombre: '',
        apellido: '',
        telefono: '',
        email: '',
    ...
    const [officeData, setOfficeData] = useState({
        ...
        medico: initialMedico, // Inicializamos con un médico vacío
    });

    const [createNewMedico, setCreateNewMedico] = useState(false);
    const [selectExistingMedico, setSelectExistingMedico] = useState(false);
    const [existingMedicos, setExistingMedicos] = useState<Medico[]>([]); // Lista de médicos existentes
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            alert(
                'No se pudo guardar el consultorio. Por favor, inténtelo nuevamente.'
            );
        }
    };
