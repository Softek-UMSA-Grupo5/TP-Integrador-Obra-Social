...
const ConsultorioForm: React.FC = () => {
   ...
   const handleLocationChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === 'altura') {
        // Parseamos el valor de 'altura' a un número o dejamos vacío si es inválido
        const alturaValue: number | '' = value === '' ? '' : parseInt(value, 10);

        setOfficeData((prevData) => ({
            ...prevData,
            ubicacion: {
                ...prevData.ubicacion,
                altura: isNaN(alturaValue) ? '' : alturaValue,
            },
        }));
    } else {
        setOfficeData((prevData) => ({
