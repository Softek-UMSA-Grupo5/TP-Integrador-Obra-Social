...
const ConsultorioForm: React.FC = () => {
    ...
    const handleHorarioChange = (index: number, event: ChangeEvent<HTMLInputElement>) => {
        ...
        // Obtener la hora actual del horario
        const currentHorario = officeData.horarioAtencion[index];

        // Formatear la hora ingresada
        ...
        const updatedHorarioAtencion = officeData.horarioAtencion.map((horario, i) =>
            i === index ? { ...currentHorario, [name]: formattedTime } : horario
        );
    ...
    const formatTimeInput = (value: string): string => {
        ...
        // Divide la entrada para separar horas y minutos
        const parts = cleanedValue.split(':');

        // Formatea las horas, asegurándose de que no sea mayor a 23 y tenga dos dígitos
        const hours = parts[0]
            ? String(Math.min(parseInt(parts[0], 10), 23)).padStart(2, '0')
            : '00';

        // Si hay minutos, formatea los minutos asegurándose de tener dos dígitos, de lo contrario, establece los minutos en '00'
        const minutes = parts[1]
            ? String(Math.min(parseInt(parts[1], 10), 59)).padStart(2, '0')
            : '00';

        // Si hay segundos, formatea los segundos asegurándose de tener dos dígitos, de lo contrario, establece los segundos en '00'
        const seconds = parts[2]
            ? String(Math.min(parseInt(parts[2], 10), 59)).padStart(2, '0')
            : '00';
