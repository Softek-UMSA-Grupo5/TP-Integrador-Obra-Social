...
const ConsultorioForm: React.FC = () => {
    ...
    // Formatea la entrada para asegurar que solo contenga números y ':'
    const formattedValue = formatTimeInput(value);

    // Actualiza el horario según el nombre del campo
    const updatedHorarioAtencion = officeData.horarioAtencion.map((horario, i) =>
        i === index ? { ...horario, [name]: formattedValue } : horario
    );

    // Actualiza el estado con el nuevo horario formateado
    setOfficeData((prevData) => ({
        ...prevData,
        horarioAtencion: updatedHorarioAtencion.map((horario) => ({
            ...horario,
            horaInicio: name === 'horaInicio' ? addSecondsToTime(horario.horaInicio) : horario.horaInicio,
            horaFin: name === 'horaFin' ? addSecondsToTime(horario.horaFin) : horario.horaFin,
        })),
    }));
};
   ...
   const formatTimeInput = (value: string): string => {
    ...
    // Divide la entrada en partes (horas, minutos y segundos)
    const parts = cleanedValue.split(':');
    let hours = parts[0] ? String(Math.min(parseInt(parts[0], 10), 23)).padStart(2, '0') : '00';
    let minutes = parts[1] ? String(Math.min(parseInt(parts[1], 10), 59)).padStart(2, '0') : '00';
    let seconds = parts[2] ? String(Math.min(parseInt(parts[2], 10), 59)).padStart(2, '0') : '00';

    // Retorna el formato HH:mm:ss
    return `${hours}:${minutes}:${seconds}`;
};

const addSecondsToTime = (time: string): string => {
    // Si no hay segundos especificados, los agrega como ':00'
