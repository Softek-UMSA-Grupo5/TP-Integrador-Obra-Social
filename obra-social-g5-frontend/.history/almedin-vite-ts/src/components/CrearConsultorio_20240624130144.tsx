...
const ConsultorioForm: React.FC = () => {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const consultorioResponse = await createConsultorio(officeData);
            ...
            if (createNewMedico) {
                ...
                const medicoToCreate: MedicoCreateRequest = {
                    ...medicoData, // Asegúrate de tener los datos correctos del médico
                    consultoriosId: [consultorioId],
                };
                const medicoResponse = await createMedico(medicoToCreate);
                const medicoId = medicoResponse.data.id;

                // Paso 3: Actualizar el consultorio con el medicoId (si es necesario en tu lógica)
                // En tu caso, si el backend maneja la relación automáticamente, no necesitarías hacer esto
            ...
            onSubmitSuccess(); // Llamar al callback de éxito del formulario
        } catch (error) {
