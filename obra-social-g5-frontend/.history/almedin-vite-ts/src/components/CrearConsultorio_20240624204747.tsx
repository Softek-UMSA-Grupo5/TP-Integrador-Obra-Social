...
const ConsultorioForm: React.FC = () => {
    ...
    const handleHorarioChange = (index: number, event: ChangeEvent<HTMLInputElement>) => {
        ...
        setOfficeData((prevData) => ({
            ...
            horarioAtencion: updatedHorarioAtencion.map((horario) => ({
                ...horario,
                horaInicio: name === 'horaInicio' ? addSecondsToTime(value) : horario.horaInicio,
                horaFin: name === 'horaFin' ? addSecondsToTime(value) : horario.horaFin,
            })),
        }));
    };
    ...
    const formatTimeInput = (value: string): string => {
        ...
        const minutes = parts[1] ? String(Math.min(parseInt(parts[1], 10), 59)).padStart(2, '0') : '';
