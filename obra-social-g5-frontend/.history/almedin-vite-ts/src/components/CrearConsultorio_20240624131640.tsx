...
const ConsultorioForm: React.FC = () => {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const consultorioId = consultorioResponse.id; // Asumiendo que el ID se recibe directamente en la respuesta

            // Paso 2: Solo si se está creando un nuevo médico
            if (createNewMedico) {
                // Crear el médico con el consultorioId asociado
                const medicoToCreate: MedicoRequest = {
                    ...officeData, // Ajusta según cómo quieres mapear los datos
                    consultoriosId: [consultorioId],
                };
                const medicoResponse = await createMedicoAndAssociateToConsultorio(medicoToCreate, consultorioId);
                const medicoId = medicoResponse.id; // Asumiendo que el ID se recibe directamente en la respuesta
            ...
            // Aquí podrías manejar el éxito del envío del formulario si es necesario
        } catch (error) {
