...
const UsuarioForm: React.FC = () => {
    ...
    try {
        ...
        } else if(usuarioResponse.id==null) {
            // Register the user if they don't exist
            usuarioId = await registrarUsuario(usuarioData, selectedRol);
        }

        // Update medico if needed
        if (isExistingMedico && selectedMedicoId !== undefined) {
            const medicoExistente = medicos.find((medico) => medico.id === selectedMedicoId);
            if (medicoExistente?.consultoriosId && !medicoExistente.consultoriosId.includes(selectedConsultorioId!)) {
            ...
            // Add new medico
            const medicoNuevo = { ...medicoData, consultoriosId: [selectedConsultorioId!], usuarioId };
            await addMedico([medicoNuevo]);
        }

        setSuccess(true);
    } catch (error) {
        setError('Error al registrar el usuario o actualizar el médico');
