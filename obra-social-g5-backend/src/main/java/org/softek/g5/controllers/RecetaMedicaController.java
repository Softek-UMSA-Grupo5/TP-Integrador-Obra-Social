...
public class RecetaMedicaController {
	...
	public Response getAll() throws CustomServerException{
	        Collection<RecetaMedicaResponseDto> recetas = recetaMedicaService.getRecetaMedica();
	...
	@APIResponses({
	    @APIResponse(responseCode = "200", description = "Receta medica encontrada"),
	    @APIResponse(responseCode = "500", description = "Error al obtener la receta médica")
	})
	public Response getRecetaMedicaById(@Parameter(required = true, description = "id de la receta médica") @PathParam("id") Long idReceta) throws CustomServerException{
	        RecetaMedicaResponseDto receta = recetaMedicaService.getRecetaMedicaById(idReceta);
	...
	public Response getRecetaMedicaByCodigo(@Parameter(required = true, description = "código de la receta médica") @PathParam("codigo") String codigoReceta) throws CustomServerException{
	        RecetaMedicaResponseDto receta = recetaMedicaService.getRecetaMedicaByCodigo(codigoReceta);
	        return Response.ok(receta).build();
	}
	...
	public Response getRecetaMedicaBetweenDates(@Parameter(required = true, description = "fecha desde") @QueryParam("fechaDesde") String fechaDesde
			,@Parameter(required = true, description = "fecha hasta") @QueryParam("fechaHasta") String fechaHasta) throws CustomServerException{
	        List<RecetaMedicaResponseDto> recetas = recetaMedicaService.getRecetaMedicaBetweenDates(fechaDesde, fechaHasta);
	        return Response.ok(recetas).build();
	}
	...
	public Response addRecetaMedica(@Valid RecetaMedicaRequestDto dto,
			@Parameter(required = true, description = "código turno medico") @QueryParam("codigoTurno") String codigoTurno) throws CustomServerException {
	        RecetaMedicaResponseDto response = recetaMedicaService.persistRecetaMedica(codigoTurno, dto);
	...
	public Response update(@Parameter(required = true, description = "codigo de la receta médica") @PathParam("codigo") String codigoReceta
			, @Valid RecetaMedicaRequestDto dto) throws CustomServerException {
	        recetaMedicaService.updateRecetaMedica(codigoReceta, dto);
	...
	public Response delete(@Parameter(required = true, description = "código de la receta médica") @PathParam("codigo") String codigo) throws CustomServerException{
