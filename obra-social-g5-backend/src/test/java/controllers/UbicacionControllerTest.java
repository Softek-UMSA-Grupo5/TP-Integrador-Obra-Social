...
public class UbicacionControllerTest {
	...
	public static void setUp() {
		...
		try {
			ubicacionResponseDto = given().contentType(ContentType.JSON).body(ubicacionDto).when().post("/ubicaciones")
					.then().statusCode(201).extract().as(UbicacionResponseDto.class);
		} catch (Exception e) {
			e.printStackTrace();
